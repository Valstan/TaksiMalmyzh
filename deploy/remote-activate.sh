...
# Оставляем три последних релиза: откат — это переключение симлинка назад, и
# откатываться должно быть куда; но диск бокса — общий дефицит на четверых (~77 МБ на
# релиз), и мандат обслуживания 30.08 просит держать «текущий плюс один-два».
