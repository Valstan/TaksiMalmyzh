...
// Пакет внутри пакета — признак того, что трассировщик снова затянул прошлый `release/`
// в `.next/standalone` (разбор — `scripts/clean-release.mjs`). Само по себе это стоит
// копейки, поэтому и жило незамеченным до восьми уровней вложенности; ловим здесь, чтобы
// следующий раз увидеть сразу.
if (existsSync(path.join(OUT, OUT))) {
  console.error(`✗ в пакете лежит вложенный ${OUT}/ — сборка шла поверх прошлого пакета`);
  console.error("  вылечит `npm run build` заново: шаг prebuild убирает release/ перед сборкой");
  process.exit(1);
}
console.log(`· проверено: вложенного ${OUT}/ в пакете нет`);
